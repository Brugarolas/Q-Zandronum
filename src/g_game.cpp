...
CVAR( Bool, sv_useteamstartsindm, false, CVAR_SERVERINFO | CVAR_GAMEMODESETTING )

// [BB] In cooperative game modes players are spawned at random player starts instead of the one designated for them.
// [AK] Added CVAR_GAMEMODESETTING.
CVAR( Bool, sv_randomcoopstarts, false, CVAR_SERVERINFO | CVAR_GAMEMODESETTING )
...
void G_BuildTiccmd (ticcmd_t *cmd)
{
	...
	*cmd = *base;
	...
	if (Button_Speed.bDown ^ (int)cl_run)			cmd->ucmd.buttons |= BT_SPEED; // [geNia] Always consider BT_SPEED pressed when running
...
*/
	...
	for ( ULONG i = 0; i < TEAM_GetNumAvailableTeams( ); i++ )
	{
		if ( TEAM_UpdateSpecialItemOrigin( i ) )
		{
	...
	for ( unsigned int i = 0; i < existingActors.Size(); ++i )
	{
		...
		if( pNewActor != NULL ){
			...
			else if ( NETWORK_GetState( ) == NETSTATE_SERVER )
			{
				...
				SERVERCOMMANDS_UpdateThingFlagsNotAtDefaults( pNewActor );
				...
				if ( pNewActor->RenderStyle.AsDWORD != pNewActor->GetDefault()->RenderStyle.AsDWORD )
					SERVERCOMMANDS_SetActorProperty( pNewActor, APROP_RenderStyle, pNewActor->RenderStyle.AsDWORD );
